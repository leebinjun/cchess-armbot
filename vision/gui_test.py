...
class GUI(Frame):
    ...
    def __init__(self, master, **kw):
        ...
        self.__points_t = config_v.POS_BOARD
        self.__points = []  
        self.__num = 0
        ...
        self.btn3 = tk.Button(root, text="保存找圆参数", command=self.do_save_circle_parames)
        self.btn3.place(x=950, y=400)
        ...
        self.sc3.place(x=950, y=170)
        self.sc3.set(c)

        self.var_d = tk.IntVar()
        self.sc4 = tk.Scale(frame, 
                            label='minRadius',                # 设置标签内容
                            from_=10,                         # 设置最大值
                            to=100,                           # 设置最小值
                            orient=tk.HORIZONTAL,             # 设置水平方向
                            length=200,                       # 设置轨道的长度
        ...
        self.sc4.place(x=950, y=230)
        self.sc4.set(d)
        
        self.var_e = tk.IntVar()
        self.sc5 = tk.Scale(frame, 
                            label='maxRadius',                # 设置标签内容
                            from_=10,                         # 设置最大值
                            to=100,                           # 设置最小值
                            orient=tk.HORIZONTAL,             # 设置水平方向
                            length=200,                       # 设置轨道的长度
    ...
    def video_loop(self):
        ...
        if success:
            ...
            img_t = perTrans_chess(self.img, self.__points_t)
            img_t_c, _ = find_circles(img_t,
            ...
            cv2image = cv2.cvtColor(img_t_c, cv2.COLOR_BGR2RGBA) # 转换颜色从BGR到RGBA
            ...
            # 显示存子位置
                ...
                if (self.flag_recognize_chess 
                   ...
                   and type(circles_b) is not None.__class__): 
                    ...
                    alist_b = self.recognize_chess(img_b, circles_b, is_save=False)
        ...
        with open(r".\vision\config_v.py", "r", encoding='utf-8') as f:
            for line in f:
                if 'minDist' in line:
                    ...
                    line = new_line
                if 'param1' in line:
                    new_line = "param1 = " + str(self.var_b.get()) + '\n'
                    line = new_line
                if 'param2' in line:
                    new_line = "param2 = " + str(self.var_c.get()) + '\n'
                    line = new_line
                if 'minRadius' in line:
                    new_line = "minRadius = " + str(self.var_d.get()) + '\n'
                    line = new_line
                if 'maxRadius' in line:

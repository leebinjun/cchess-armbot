...
class Armbot(ComThread):
    ...
    def reset_all_servo(self):
        ...
        d=bytes.fromhex('23 31 50 31 35 30 30 23 32 50 31 35 30 30 23 33 50 31 35 30 30 23 34 50 31 35 30 30 54 31 30 30 30 0D 0A 0D 0A')
        self.l_serial.write(d)
        return self.get_ok()
    
    def one_servo_to_pos(self, servo_id, servo_pos):
        print("servo %d get to position %d" % (servo_id, servo_pos))
        d = '#' + str(servo_id) + 'P' + str(servo_pos) + 'T1000\r\n'
        print(d)
    ...
    def go_air_pump(self, signal = 0):
        print("go_air_pump")
        if signal == 0: # 停止
            d = '#4P1200#5P1500T1000\r\n'
        if signal == 1: # 吸气
            d = '#4P1800#5P1200T1000\r\n'
        if signal == 2: # 吹气
            d = '#4P1800#5P1800T1000\r\n'
        print("d:", d)
        self.l_serial.write(d.encode())
        return self.get_ok()
    
    def go_ready(self):
        d = '#1P2000#2P2000#3P1500T1000\r\n'
        print("d:", d)
        self.l_serial.write(d.encode())
        return self.get_ok()
    ...
    def go_to_pos(self, x, y, z):
        ...
        d = '#1P' + str(s1) + '#2P' + str(s2) + '#3P' + str(s3) + 'T1000\r\n'
        print("d:", d)
        self.l_serial.write(d.encode())
        time.sleep(0.5)
        return self.get_ok() 
...
if __name__ == '__main__':
    ...
    while(temp_a != 99):
        ...
        if temp_a == 1:
            ...
            myarm.one_servo_to_pos(input_x, input_y)
        elif temp_a == 6:
